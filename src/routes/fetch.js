  ...
  try {
    ...
    for (const saleOrderElement of saleOrder) {
      const values = [
        ...
        new Date(saleOrderElement?.displayOrderDateTime).toISOString(),
        saleOrderElement?.channel || null,
        saleOrderElement?.channelProcessingTime || null,
        saleOrderElement?.status || null,
        new Date(saleOrderElement.created).toISOString(),
        new Date(saleOrderElement?.updated).toISOString(),
...
`;
      ...
      const saleOrderItemValues = [
        ...
        currencyCode,
        taxPercentage,
        new Date(fulfillmentTat).toISOString(),
        totalPrice,
        sellingPrice,
        prepaidAmount,
        discount,
  ...
  `;
    ...
    for (let saleOrderElement of saleOrder) {
      ...
      const shippingPackagesValues = [
        ...
        new Date(shippingPackagesObject?.created).toISOString() || null,
        new Date(shippingPackagesObject?.updated).toISOString() || null,
